        ...
        child: Wrap(
          ...
          children: [
            ...
            _ToolButton(icon: Icons.code, label: 'Code',
                onPressed: () => _wrapSelection('`', '`')),
            _ToolButton(icon: Icons.link, label: 'Link', onPressed: () {
              final text = controller.text;
              final sel = controller.selection;
              final start = sel.start;
              final end = sel.end;
            ...
            _ToolButton(icon: Icons.title, label: 'H1',
                onPressed: () => _insertAtLineStart('#')),
            _ToolButton(icon: Icons.title, label: 'H2',
                onPressed: () => _insertAtLineStart('##')),
            _ToolButton(icon: Icons.format_list_bulleted, label: '• List',
                onPressed: () => _insertAtLineStart('-')),
            _ToolButton(icon: Icons.format_list_numbered, label: '1. List',
                onPressed: () => _insertAtLineStart('1.')),
            _ToolButton(icon: Icons.horizontal_rule, label: 'Rule', onPressed: () {
              ...
              controller.value = controller.value.copyWith(
                text: newText,
                selection: TextSelection.collapsed(offset: (sel.start + insert.length).clamp(0, newText.length)),
              );
              onChanged?.call();
            }),
          ],
